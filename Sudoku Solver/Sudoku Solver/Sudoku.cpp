...
const void Sudoku::setGrid(const int v[9][9])
{
	...
	//punem posibilitatile comune ale patratelor si liniilor
	fillSharedPossibilities();

	//punem pisibilitatile pentru fiecare pozitie in grid
	fillPossibilities();
}
...
const bool Sudoku::checkInSquare(int x, int y, int v)
{
	...
	for(int i = 0; i < 3; ++i)
		for (int j = 0; j < 3; ++j)
		{
			//nu verificam insusi valorile
			if (_x + i == x && _y + j == y)
				continue;

			//verificam daca exista valoarea
			if (getValue(_x + i, _y + i) == v)
				return false;

			//verificam daca este problema posibilitatii unice
