...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		{
			//luam imaginea de input pentru a resolva sudoku
			img_src = imread(argv[2], IMREAD_UNCHANGED);

			//verificam sursa imaginii
			if (img_src.empty()) throw String("the image" + string(argv[2]) + " cannot be load");
		...
		else if (argc >= 4 && !strcmp(argv[1], "-t"))
		{
			...
			while (++i < argc)
			{
				//antrenam data fiecarui file
				trainOCR(argv[i], sample, response);

				//salvam exemplele antrenate si raspunsurile in ultimul filename
		...
		else if (argc >= 4 && !strcmp(argv[1], "-tl"))
		{
			...
			while (++i < argc)
			{
				//citim data antrenata
				if (i == 2) loadTrainedOCR(argv[2], sample, response);
				//antrenam data pt fiecare fisier
				else
				{
					trainOCR(argv[i], sample, response);

					//salvam exemplele si raspunsurile antrenate in ultimul fisier
					if (i == argc - 1) saveTrainedOCR(argv[2], sample, response);
				}
			}
			return 0;
	...
	//remove orig number from the solved grid for no drawind it
	for(int y = 0; y < 9; ++y)
		for (int x = 0; x < 9; ++x)
		{
			if (grid[x][y] != 0)
