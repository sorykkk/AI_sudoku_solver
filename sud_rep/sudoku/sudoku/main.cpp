...
int main(int argc, char* argv[])
{
	...
	try
	{
		if (argc == 2)
		{
			...
			img_src = imread("C:\\Users\\soryn\\Desktop\\sud_rep\\sudoku\\sudoku\\images\\test1.jpg", IMREAD_UNCHANGED);

			//check the image source
			if (img_src.empty()) throw String("the image " + string(argv[1]) + " cannot be loaded");

			loadTrainedOCR("C:\\Users\\soryn\\Desktop\\sud_rep\\sudoku\\sudoku\\OCR-data.yaml", sample, response);
		}
	...
	// solve the sudoku
	if (sudokuSolve(su, 0, 0))///***SCHIMBAM AICI***///
	{
		// get the solved grid
